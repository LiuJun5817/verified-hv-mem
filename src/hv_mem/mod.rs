...
impl SpecMemRegion {
    ...
    pub open spec fn translate(&self, v: SpecVAddr) -> SpecPAddr
        recommends
            self.contains_v(v),
    {
        SpecPAddr((self.pstart.0 + (v.0 - self.vstart.0) as nat))
    }
}
...
impl SpecHvMem {
    ...
    pub open spec fn p1_region_disjoint(&self) -> bool {
        ...
        &&& forall|z: int, i: int, j: int|
            0 <= z < self.zones.len() && 0 <= i < self.zones[z].cfg_regions.len() && 0 <= j
                < self.zones[z].cfg_regions.len() && i < j
                ==> self.zones[z].cfg_regions[i].phys_disjoint(
                &self.zones[z].cfg_regions[j],
            )
        // Inter-zone disjointness.
        &&& forall|z1: int, z2: int, i: int, j: int|
            0 <= z1 < self.zones.len() && 0 <= z2 < self.zones.len() && z1 < z2 && 0 <= i
                < self.zones[z1].cfg_regions.len() && 0 <= j < self.zones[z2].cfg_regions.len()
    ...
    pub open spec fn p2_zone_isolated(&self) -> bool {
        forall|z: int, v: SpecVAddr|
            0 <= z < self.zones.len() && self.zones[z].mem_set.view.contains_key(v) ==> ({
    ...
    pub open spec fn p3_ptmem_disjoint(&self) -> bool {
        &&& forall|i: int, j: int|
            0 <= i < j < self.zones.len() ==> self.zones[i].pt_frames.disjoint(
                self.zones[j].pt_frames,
            )
        &&& forall|zi: int|
            0 <= zi && zi < self.zones.len() ==> ({
                let z = #[trigger] self.zones[zi];
                self.alloc.clients.contains_key(z.zone_id) && z.pt_frames.subset_of(
                    self.alloc.clients.index(z.zone_id),
                )
            })
    }
