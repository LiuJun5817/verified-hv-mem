...
pub trait BitAllocView {
    ...
    fn find_contiguous(&self, capacity: usize, size: usize, align_log2: usize) -> (res: Option<
        usize,
    >)
        requires
            self.wf(),
        ...
        ensures
            ...
            match res {
                Some(base) => {
                    ...
                    &&& forall|i: int|
                        base <= i < base + size ==> self@[i]
                            == true  //self.next(i) != None

                },
                None => {
                    // If failed, no suitable block exists.
                    // This implies either no free bits, or all potential blocks are obstructed or misaligned.
                    capacity < (1usize << align_log2) || !self.spec_any() || forall|i: int|
                        (0 <= i <= capacity - size) ==> has_obstruction(
                            self@,
                            i,
                            size as int,
...
pub trait BitAlloc: BitAllocView {
    ...
    fn set_range_to(&mut self, range: Range<usize>, val: bool)
        ...
        ensures
            forall|loc1: int| (range.start <= loc1 < range.end) ==> self@[loc1] == val,
            forall|loc2: int|
    ...
    fn remove(&mut self, range: Range<usize>)
        requires
            ...
            range.start < range.end,
        ensures
            forall|loc1: int| (range.start <= loc1 < range.end) ==> self@[loc1] == false,
            forall|loc2: int|
                (0 <= loc2 < range.start || range.end <= loc2 < Self::spec_cap()) ==> self@[loc2]
                    == old(self)@[loc2],
            self@.len() == old(self)@.len(),
...
proof fn set_bits_u16_preserves_others(bv_new: u16, bv_old: u16, st: u16, ed: u16, val: u16)
    requires
        ...
        val << ((u16::BITS) as u16 - (ed - st)) as usize >> ((u16::BITS) as u16 - (ed
            - st)) as usize == val,
        bv_new == set_bits16!(bv_old, st, ed, val),
    ensures
        get_bits16!(bv_new, st, ed) == val,
        forall|loc2: u16|
            (0 <= loc2 < st || ed <= loc2 < 16) ==> (get_bit16!(bv_new, loc2)
                == get_bit16!(bv_old, loc2)),
{
}
...
proof fn get_bits_u16_correctness(bv_gets: u16, bits: u16, st: u16, ed: u16)
    ...
    ensures
        forall|i: u16|
            0 <= i < (ed - st) ==> ((get_bit16!(bv_gets, i)) == get_bit16!(bits, (st + i) as u16)),
{
}
...
impl<T: BitAllocView + Copy> BitAllocView for BitAllocCascade16<T> {
    open spec fn view(&self) -> Seq<bool> {
        // Concatenate the views of the 16 child allocators.
        ...
        Seq::new(
            ...
            |idx: int|
                {
                    let i = idx / sub_len;  // Child allocator index.
                    let off = idx % sub_len;  // Offset within the child allocator.
                    self.sub[i as int]@[off as int]  // Read the corresponding bit.

                },
        )
    }

    // Every child allocator has the same fixed capacity.
    fn cap() -> (res: usize) {
        (T::cap() * 16) as usize
    }
    ...
    proof fn lemma_bits_nonzero_implies_exists_true(&self) {
        ...
        if self.spec_any() {
            ...
            assert(exists|j: int| 0 <= j < Self::spec_cap() && self@[j] == true) by {
                let j = loc;
            };
        } else {
            // self.bitset.bits == 0
            ...
            assert forall|k: int, j: int| 0 <= k < 16 && 0 <= j < cap implies self.sub[k]@[j]
                == false by {
            ...
            assert forall|loc2: int| 0 <= loc2 < Self::spec_cap() implies self@[loc2] == false by {
                ...
                assert(0 <= j < cap && 0 <= k < 16 && loc2 == k * cap + j) by (nonlinear_arith)
                    requires
                        k == loc2 / cap,
                        j == loc2 % cap,
    ...
    fn test(&self, key: usize) -> (res: bool) {
        let seq_index: usize = key / T::cap();  // Prove seq_index < 16.
    ...
    open spec fn wf(&self) -> bool {
        ...
        &&& forall|k: int|
            0 <= k < 16
                ==> self.sub[k].wf()
        // Parent bit i is set exactly when child i has a free bit.
        &&& forall|k: int|
            0 <= k < 16 ==> self.bitset@[k]
                == self.sub[k].spec_any()
        // Map each parent-view segment to its child view.
    ...
    fn next(&self, key: usize) -> (res: Option<usize>) {
        ...
        while i < 16
            ...
            invariant
                ...
                self.wf(),
                forall|k: int|
                    key <= k < curr_key ==> self@[k]
                        == false,
        // All bits checked from key to curr_key are unavailable.

            ensures
                (i == 16 && result.is_none() && curr_key == Self::spec_cap()) || (i < 16
                    && result.is_some() && (result.unwrap() == curr_key as usize) && curr_key
                    < Self::spec_cap() && curr_key >= key && self@[result.unwrap() as int] == true),
            decreases 16 - i,
        {
            if self.bitset.get_bit(i as u16) {
                ...
                // The well-formed invariant establishes the 16-way tree shape.
                ...
                assert(forall|t: int|
                    0 <= t < T::spec_cap() as int ==> self.sub[i as int]@[t] == false);
                proof {
                    lemma_view_indexs_st_to_ed_mapping_false(
                        self@,
                        i as int,
            ...
            assert(forall|t: int|
                key as int <= t < (T::spec_cap() * i + T::spec_cap()) as int ==> self@[t] == false);
            let old_i = i;
            ...
            assert(T::spec_cap() * (old_i + 1) == (T::spec_cap() * i + T::spec_cap()))
                by (nonlinear_arith)
                requires
                    old_i == i,
            ;
    ...
    >) {
        ...
        while offset < capacity
            invariant
                ...
                forall|i: int|
                    (0 <= i < capacity) && (i % (1usize << align_log2) as int != 0)
                        ==> has_obstruction(self@, i, size as int, (1usize << align_log2) as int),
                capacity == Self::spec_cap(),
                align_log2 < 64,
                0 < size <= capacity,
                0 <= base <= offset,
            ...
            if let Some(next) = self.next(offset) {
                if next != offset {
                    ...
                    assert(forall|i: usize|
                        (offset - size < i <= offset) ==> has_obstruction(
                            self@,
                            i as int,
                            size as int,
                            (1usize << align_log2) as int,
                    ...
                    assert forall|i: usize| (offset <= i < next) implies has_obstruction(
                        self@,
                        i as int,
                        size as int,
                        (1usize << align_log2) as int,
                    ...
                    assert forall|i: usize| (next <= i < base) implies has_obstruction(
                        self@,
                        i as int,
                        size as int,
                        (1usize << align_log2) as int,
                ...
                assert forall|i: usize| (offset <= i < capacity) implies has_obstruction(
                    self@,
                    i as int,
                    size as int,
                    (1usize << align_log2) as int,
...
impl<T: BitAlloc + Copy> BitAlloc for BitAllocCascade16<T> {
    fn alloc(&mut self) -> (res: Option<usize>) {
        ...
        if !self.any() {
            ...
            assert forall|loc: int| 0 <= loc < Self::spec_cap() implies self@[loc] == false by {
                if self@[loc] {
                    assert(exists|j: int| 0 <= j < Self::spec_cap() && self@[j] == true) by {
                        let j = loc;
                    };
        ...
        // A cleared parent bit means that the corresponding child is full.
        // Every child before i is full.
        ...
        assert(forall|loc2: int|
            (0 <= loc2 < Self::spec_cap()) ==> self@[loc2] == self.sub[loc2 / cap]@[loc2 % cap]);

        assert(forall|j: int| 0 <= j < 16 && j != i ==> self.sub[j]@ == old(self).sub[j]@);

        assert(self.sub[i as int]@ == old(self).sub[i as int]@.update(
            res_is_some.unwrap() as int,
            false,
        ));
        ...
        // Show that all other bits in the parent view are unchanged.
        assert forall|loc2: int|
            (0 <= loc2 < Self::spec_cap() && loc2 != (i * cap
        ...
        // The parent view changes only at the newly allocated bit.
        proof {
            assert_seqs_equal!(
                self@,
        ...
        // Show that the old parent-view prefix before res was unavailable.
        assert(forall|j: int|
            0 <= j < res_is_some.unwrap() as int ==> old(self).sub[i as int]@[j] == false);
        assert forall|loc2: int|
            ...
            assert(forall|loc2: int|
                (0 <= loc2 < Self::spec_cap()) ==> old(self)@[loc2] == old(self).sub[loc2
                    / cap]@[loc2 % cap]);
        };
    ...
    fn alloc_contiguous(&mut self, size: usize, align_log2: usize) -> (res: Option<usize>) {
        ...
        if let Some(base) = self.find_contiguous(Self::cap(), size, align_log2) {
            ...
            if !self.any() {
                proof {
                    self.lemma_bits_nonzero_implies_exists_true();
                    assert(no_available(self@, Self::spec_cap() as int));
                    lemma_no_available_implies_all_obstructed(
                        self@,
                        Self::spec_cap(),
                        size,
                        1usize << align_log2,
    ...
    fn dealloc(&mut self, key: usize) {
        ...
        // Show that all other bits in the parent view are unchanged.
        assert forall|loc2: int|
            (0 <= loc2 < Self::spec_cap() && loc2 != key as int) implies self@[loc2] == old(
            self,
        )@[loc2] by {
            let j = loc2 / cap;
            let k = loc2 % cap;
        ...
        // The parent view changes only at the freed bit.
        proof {
            assert_seqs_equal!(
                self@,
    ...
    fn set_range_to(&mut self, range: Range<usize>, val: bool) {
        ...
        let first = st / T::cap();  // First affected child.
        let last = (ed - 1) / T::cap();  // Last affected child.
        let n = last + 1;  // Exclusive loop bound.
        ...
        while i < n
            invariant
                ...
                ed <= Self::spec_cap(),
                T::spec_cap() > 0,
                current_end == if i == st / T::spec_cap() {
                    st
                } else if i == n as int {
                    ed
                } else {
                    (i * T::spec_cap()) as usize
                },
                forall|loc1: int| st <= loc1 < current_end ==> self@[loc1] == val,
                forall|loc2: int|
                    (0 <= loc2 < st || current_end <= loc2 < Self::spec_cap()) ==> self@[loc2]
                        == old(self)@[loc2],
                forall|loc3: int|
                    (0 <= loc3 < first || i <= loc3 < 16) ==> self.sub[loc3]@ == old(
                        self,
                    ).sub[loc3]@,
                self@.len() == old(self)@.len(),
                self.wf(),
            decreases n - i,
        {
            ...
            assert(forall|loc1: int|
                0 <= loc1 < Self::spec_cap() ==> pre_self[loc1] == self.sub[loc1 / cap as int]@[loc1
                    % cap as int]);
            assert forall|k: int, j: int| 0 <= k < 16 && 0 <= j < cap implies self@[k * cap + j]
                == self.sub[k]@[j] by {
                let loc1 = k * cap + j;
                assert(k == loc1 / cap as int && j == loc1 % cap as int) by (nonlinear_arith)
                    requires
                        loc1 == k * cap + j,
                        cap > 0,
                        0 <= j < cap,
            ...
            assert(forall|k: int, j: int|
                0 <= k < 16 && 0 <= j < cap ==> pre_self[k * cap + j] == self.sub[k]@[j]);

            // Preserve the untouched suffix in pre_self.
            assert(forall|loc2: int|
                (0 <= loc2 < st || current_end <= loc2 < Self::spec_cap()) ==> pre_self[loc2]
                    == old(self)@[loc2]);
            ...
            assert(0 <= begin < cap) by (nonlinear_arith)
                requires
                    ...
                    cap > 0,
            ;
            assert forall|loc1: int| (i + 1) * cap <= loc1 < Self::spec_cap() implies pre_self[loc1]
                == old(self)@[loc1] by {
                ...
                ;
                assert(forall|loc2: int|
                    (current_end <= loc2 < Self::spec_cap()) ==> pre_self[loc2] == old(
                        self,
                    )@[loc2]);
            ...
            // Restore the parent-child well-formed invariant.
            assert(self.bitset@[i as int] == self.sub[i as int].spec_any());
            assert(forall|k: int| 0 <= k < 16 ==> self.sub[k].wf());
            ...
            assert(forall|loc1: int|
                0 <= loc1 < Self::spec_cap() ==> self@[loc1] == self.sub[loc1 / cap as int]@[loc1
                    % cap as int]);

            // Updating child i changes only its segment.
            assert(forall|k: int, j: int|
                0 <= k < 16 && k != i && 0 <= j < cap ==> pre_self[k * cap + j] == self.sub[k]@[j]);

            assert forall|loc1: int|
                0 <= loc1 < i * cap || (i + 1) * cap <= loc1
                    < Self::spec_cap() implies pre_self[loc1] == self.sub[loc1 / cap as int]@[loc1
                % cap as int] by {
                let k = loc1 / cap;
                let j = loc1 % cap;
            ...
            assert forall|loc1: int|
                0 <= loc1 < i * cap || (i + 1) * cap <= loc1 < Self::spec_cap() implies self@[loc1]
                == self.sub[loc1 / cap as int]@[loc1 % cap as int] by {
            ...
            assert(forall|loc1: int|
                0 <= loc1 < i * cap || (i + 1) * cap <= loc1 < Self::spec_cap() ==> self@[loc1]
                    == pre_self[loc1]);

            // The processed prefix from st to current_end has value val.
            assert forall|loc1: int| st <= loc1 < current_end implies self@[loc1] == val by {
                if i == first {
                    ...
                    ;
                    assert(forall|loc2: int|
                        st <= loc2 < current_end ==> self@[loc2] == pre_self[loc2]);
                    assert(pre_self[loc1] == val);
                    assert(self@[loc1] == val);
                }
            };

            // Prove that advancing current_end cannot overflow.
            assert(stop + i * cap <= Self::spec_cap()) by (nonlinear_arith)
                requires
                    stop == (if i == (ed - 1) / T::spec_cap() as int {
                        if ed % T::spec_cap() == 0 {
                            T::spec_cap()
                        } else {
                            ed % T::spec_cap()
            ...
            // Relate the updated child range back to the parent view.
            assert(forall|loc1: int| (begin <= loc1 < stop) ==> child@[loc1] == val);
            assert(forall|loc1: int| (begin <= loc1 < stop) ==> self.sub[i as int]@[loc1] == val);

            assert forall|loc1: int|
                ((begin + i * cap) <= loc1 < (stop + i * cap)) implies self@[loc1]
                == self.sub[i as int]@[loc1 - (i * cap) as int] by {
            ...
            assert forall|loc1: int|
                ((begin + i * cap) <= loc1 < (stop + i * cap)) implies self@[loc1] == val by {
                assert(forall|k: int| (begin <= k < stop) ==> self.sub[i as int]@[k] == val);
            };

            assert(forall|loc1: int| st <= loc1 < current_end ==> self@[loc1] == val);

            // Show that bits outside the child range are unchanged.
            assert(forall|loc1: int|
                (0 <= loc1 < begin || stop <= loc1 < T::spec_cap()) ==> child@[loc1]
                    == old_child@[loc1]);

            assert(forall|loc1: int|
                (0 <= loc1 < begin || stop <= loc1 < T::spec_cap()) ==> self.sub[i as int]@[loc1]
                    == old_child@[loc1]);

            assert(forall|loc1: int|
                (0 <= loc1 < begin || stop <= loc1 < T::spec_cap()) ==> self.sub[i as int]@[loc1]
                    == old(self).sub[i as int]@[loc1]);

            // First prove that the prefix before st is unchanged.
            assert forall|loc2: int| (0 + i * cap <= loc2 < begin + i * cap) implies self@[loc2]
                == self.sub[i as int]@[loc2 % cap as int] by {
            ...
            assert forall|loc2: int| (0 + i * cap <= loc2 < begin + i * cap) implies self@[loc2]
                == old(self).sub[i as int]@[loc2 % cap as int] by {
                let j = loc2 % cap;
            ...
            assert(forall|loc1: int|
                0 <= loc1 < Self::spec_cap() ==> old(self)@[loc1] == old(self).sub[loc1
                    / cap as int]@[loc1 % cap as int]);
            assert forall|loc2: int| (0 + i * cap <= loc2 < begin + i * cap) implies self@[loc2]
                == old(self)@[loc2] by {
                let k = loc2 / cap;
                let j = loc2 % cap;
            ...
            assert forall|loc2: int| (0 <= loc2 < st) implies self@[loc2] == old(self)@[loc2] by {
                if i == first {
                    assert(forall|loc1: int|
                        0 <= loc1 < i * cap ==> self@[loc1] == old(self)@[loc1]);
                    assert(forall|loc1: int|
                        (0 + i * cap <= loc1 < begin + i * cap) ==> self@[loc1] == old(
                            self,
                        )@[loc1]);
                    assert(self@[loc2] == old(self)@[loc2]);
                } else {
                    ...
                    assert(forall|loc1: int|
                        (0 <= loc1 < st) ==> pre_self[loc1] == old(self)@[loc1]);
            ...
            // Then prove that the suffix from current_end is unchanged.
            assert(0 <= stop <= cap) by (nonlinear_arith)
                requires
                    ...
                    cap > 0,
            ;
            assert forall|loc2: int| (stop + i * cap <= loc2 < (i + 1) * cap) implies self@[loc2]
                == self.sub[i as int]@[loc2 % cap as int] by {
            ...
            assert(forall|loc1: int|
                (i + 1) * cap <= loc1 < Self::spec_cap() ==> self@[loc1] == pre_self[loc1]);
            assert(forall|loc1: int|
                (i + 1) * cap <= loc1 < Self::spec_cap() ==> pre_self[loc1] == old(self)@[loc1]);

            assert(forall|loc2: int| (0 <= loc2 < st) ==> self@[loc2] == old(self)@[loc2]);
            assert(forall|loc2: int|
                current_end <= loc2 < Self::spec_cap() ==> self@[loc2] == old(self)@[loc2]);
            assert(forall|loc2: int|
                (0 <= loc2 < st || current_end <= loc2 < Self::spec_cap()) ==> self@[loc2] == old(
                    self,
                )@[loc2]);
            i += 1;
            if i == n {
...
impl<T: BitAllocView + Copy> BitAllocCascade16<T> {
    ...
    proof fn lemma_maintain_view_indexs_mapping(&self)
        requires
            ...
            Self::spec_cap() == 16 * T::spec_cap(),
            forall|loc2: int|
                0 <= loc2 < Self::spec_cap() ==> self@[loc2] == self.sub[(loc2
                    / T::spec_cap() as int)]@[(loc2 % T::spec_cap() as int)],
        ensures
            forall|j: int|
                0 <= j < 16 ==> view_index_mapping(self@, j, self.sub[j]@, T::spec_cap() as int),
    {
        let ghost cap = T::spec_cap() as int;
        assert forall|j: int| 0 <= j < 16 implies view_index_mapping(
            self@,
            j,
            self.sub[j]@,
            cap,
        ) by {
            assert forall|loc: int| 0 <= loc < cap implies self@[(cap * j + loc)]
                == self.sub[j]@[loc] by {
                assert(0 <= cap * j + loc < Self::spec_cap() && (cap * j + loc) / cap == j && (cap
                    * j + loc) % cap == loc) by (nonlinear_arith)
                    requires
                        Self::spec_cap() == cap * 16,
                        0 <= j < 16,
                        cap > 0,
...
pub proof fn lemma_view_indexs_st_to_ed_mapping_false(
    ba: Seq<bool>,
    i: int,
    sub_ba: Seq<bool>,
    cap: int,
...
{
    assert forall|j: int| (cap * i + start) <= j < (cap * i + end) implies ba[j] == false by {
        let t = j - cap * i;
        assert(start <= t < end && j == cap * i + t);
...
impl BitAlloc16 {
    ...
    fn get_bits(&self, range: Range<u16>) -> (bits: u16)
        ...
        ensures
            forall|i: int|
                0 <= i < (range.end - range.start) ==> (u16_view(bits)[i]) == self@[range.start
                    + i],
    {
    ...
    fn set_bits(&mut self, range: Range<u16>, value: u16)
        ...
        ensures
            ...
            forall|loc2: int|
                (0 <= loc2 < range.start || range.end <= loc2 < 16) ==> self@[loc2] == old(
                    self,
                )@[loc2],
    {
        let bv_old: u16 = self.bits;
...
impl BitAllocView for BitAlloc16 {
    ...
    proof fn lemma_bits_nonzero_implies_exists_true(&self) {
        ...
        if self.spec_any() {
            ...
            assert(exists|k: int| 0 <= k < 16 && ba[k] == true) by {
                let k = i as int;
            };
        } else {
            let i: u16 = bits.trailing_zeros() as u16;
    ...
    fn next(&self, key: usize) -> (res: Option<usize>) {
        ...
        while i < n
            ...
            ensures
                (i == n && result.is_none()) || (i < n && result.is_some() && (result.unwrap()
                    == i as usize) && self@[i as int] == true),
            decreases n - i,
        {
            if self.get_bit(i) {
...
impl BitAlloc for BitAlloc16 {
    ...
    fn insert(&mut self, range: Range<usize>) {
        ...
        assert(insert_val << (16 - width) as u16 >> (16 - width) as u16 == insert_val)
            by (bit_vector)
            requires
        ...
        assert forall|loc1: int| (st <= loc1 < ed) implies self@[loc1] == true by {
            // Normalize loc1 to its offset i from the range start.
            let i = loc1 - st;
    ...
    fn remove(&mut self, range: Range<usize>) {
        ...
        assert((16 - width) >= 0);
        assert(0u16 << (16 - width) as u16 >> (16 - width) as u16 == 0u16) by (bit_vector);
        let range_u16 = (range.start as u16)..(range.end as u16);
...
/// Macro to implement the BitmapAllocator trait for multiple types.
macro_rules! impl_bitmap_allocator { ($($ty:ty),*) => {
    $(verus!{ impl BitmapAllocator for $ty {
        open spec fn view(&self) -> Seq<bool> {
...
impl_bitmap_allocator!(BitAlloc256, BitAlloc4K, BitAlloc64K, BitAlloc1M);

pub open spec fn no_available(ba: Seq<bool>, cap: int) -> bool {
    forall|j: int| 0 <= j < cap ==> ba[j] == false
}

proof fn lemma_no_available_implies_all_obstructed(
    ba: Seq<bool>,
    cap: usize,
    size: usize,
    align: usize,
    ...
    requires
        no_available(ba, cap as int),
        0 < size <= cap,
    ensures
        forall|i: int|
            0 <= i <= (cap - size) as int ==> has_obstruction(ba, i, size as int, align as int),
{
    assert forall|i: int| 0 <= i <= (cap - size) as int implies has_obstruction(
        ba,
        i,
        size as int,
        align as int,
    ) by {
        assert(0 <= i < cap && i < i + size) by (nonlinear_arith)
            requires
                0 <= i <= (cap - size) as int,
                0 < size <= cap,
        ;
        assert(ba[i] == false);
        assert(exists|k: int| i <= k < i + size && ba[k] == false) by {
            let k = i;
        };
    };
}
...
proof fn lemma_up_align_le_capacity(next: usize, capacity: usize, align_log2: usize)
    requires
        ...
        align_log2 < 64,
        (1usize << align_log2) <= capacity,
        capacity == 16 || capacity == 256 || capacity == 4096 || capacity == 65536 || capacity
            == 1048576,
    ensures
